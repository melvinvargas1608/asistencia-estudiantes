...
import { BookOpen, LayoutDashboard, QrCode, LogOut, MenuIcon, X } from 'lucide-react'
...
export default function EstudianteLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        supabase.auth.getUser().then(async ({ data: { user } }) => {
            ...
            const { data } = await supabase
                ...
                .single()
            if (data) setStudentName(`${data.nombre} ${data.apellido}`)
        })
    }, [router])
    ...
    const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
        <aside className={`${mobile ? 'flex' : 'hidden md:flex'} flex-col w-64 min-h-screen bg-emerald-950 text-white`}>
            ...
            <div className="px-6 py-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-medium truncate max-w-[140px]">{studentName || 'Cargando...'}</p>
                    </div>
                </div>
            </div>
                ...
                {navItems.map(item => {
                    ...
                    return (
                        <Link
                            ...
                            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${active ? 'bg-emerald-600 text-white' : 'text-emerald-300 hover:bg-white/5 hover:text-white'
                                }`}
                        >
